...
for (const [stack, shellFixture] of [['vue', 'vue-app-shell.html'], ['react', 'react-app-shell.html']]) {
  test(`${stack} FdyAppShell: unbound, it defaults by viewport rather than to a guess (#8)`, { skip }, async () => {
    await withPage(fixture(shellFixture), async (p) => {
      ...
      assert.equal(wide.sidebarInert, false, 'a wide viewport starts with the nav as a visible column');

      await p.setViewport(420, 900);
      await shellUntil(p, `window.state().expanded === 'false'`, 'the shell to adopt the narrow viewport');
  ...
  test(`${stack} FdyAppShell: the overlay traps focus and gives it back, like the enhancer (#8)`, { skip }, async () => {
    await withPage(fixture(shellFixture), async (p) => {
      await shellUntil(p, '!!window.state', 'the shell to mount');
      await p.setViewport(420, 900);
      await shellUntil(p, `window.state().expanded === 'false'`, 'the shell to adopt the narrow viewport');

...
**v1.13.0 — disiapkan di working tree, BELUM di-commit/tag/push** (menunggu push berikutnya; OIDC
mem-publish saat tag `v1.13.0` naik). #37 diimplementasi & diverifikasi (gate 20/20, typecheck 0, SFC
compile, bukti browser via chrome-headless-shell); semua ref versi sudah di-bump ke 1.13.0. **Terakhir
terbit di npm = v1.12.0** (`main` = `origin/main` = tag `v1.12.0`), publik sebagai `@cahyo-dimas/freeday`.
(1.13.0 = note #37 pada `FdyDatepicker` Vue+React: **A** prop `clearable` → tombol × di trigger (overlay
di slot ikon) meng-emit `''` untuk mengosongkan tanggal opsional; **B** label nav bulan bisa di-override
(`prevMonthLabel`/`nextMonthLabel` + `clearLabel`) & default UI string kini **Inggris** (`Select date` dst,
selaras docs English-first) — enhancer vanilla tetap Indonesia. Aditif; satu perubahan default string.

...
**v1.11.1 — sudah di-release & di-push** (`main` = `origin/main` = tag `v1.11.1`). Terbit di npm
publik sebagai `@cahyo-dimas/freeday`. (1.11.1 = fix-only note #34: `.fdy-modal` tak pernah punya flex
context → body tak bisa scroll & footer/tombol submit ke-clip di viewport pendek. Diberi pola kolom-flex
milik `.fdy-drawer`: `display:flex` di `[open]`, `flex-direction:column`, `flex:none` header/footer,
`flex:1;min-height:0` body. Diverifikasi di browser incl. `.fdy-modal--cfl` — tanpa regresi.)
Sebelumnya: 1.11.0 = follow-up notes #31–#33, aditif/korektif: **#31**

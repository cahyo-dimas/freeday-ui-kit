...
**v1.13.1 — sudah di-release & di-push** (`main` = `origin/main` = tag `v1.13.1`). Terbit di npm publik
sebagai `@cahyo-dimas/freeday`. (1.13.1 = fix-only note #38: `FdyCombo` tak bisa dipilih pakai MOUSE —
opsi `<li>` tak fokusabel → `mousedown` memindah fokus dari tombol → `@focusout` root menutup list SEBELUM
`mouseup`, jadi `click` tak pernah terbit & `update:modelValue` tak emit (keyboard tetap jalan → lolos
smoke-test keyboard). Fix: `@mousedown.prevent` (Vue) / `onMouseDown` preventDefault (React) di opsi —
fokus tetap di tombol, klik mendarat. Pola sama dgn FdyDatepicker/FdyAutocomplete & vanilla
`freeday-select.js` (fix v1.6.1). Dua adapter, tanpa perubahan API. Diverifikasi CDP trusted mouse
(sintetis `dispatchEvent` tak mereproduksi). Sebelumnya 1.13.0 = note #37 pada `FdyDatepicker` Vue+React:

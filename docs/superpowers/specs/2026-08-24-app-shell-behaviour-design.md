...
assembled it wrongly enough to trap keyboard users, with content marked `inert` while the nav was
open and no way out, which is what produced `breakpoints.nav` in v1.20.0.
...
4. Tab from the last focusable in the sidebar returns to the first, trapped rather than escaped. Asserted
...
7. Crossing the breakpoint while open clears the state, with no orphaned `inert`.
